...
export default function PresentationListPage() {
  ...
  const deletingTitle = deletingId
    ? (presentations.find((p) => p.id === deletingId)?.title ?? "")
    : "";
        ...
        <Container size="lg">
          <Group h={64} justify="space-between">
            <Group gap={6}>
              ...
              <Text fw={700} size="lg" c="white" lts="-0.02em">
                ClassDir
              </Text>
            </Group>
            <Group gap="md">
      ...
      <Container size="lg" pt={80} pb="xl">
        ...
        {presentations.length === 0 ? (
          ...
          <SimpleGrid cols={{ base: 1, sm: 2, md: 3 }}>
            {presentations.map((p) => (
              <Card className={styles.card} key={p.id} bg="dark.8" padding="lg" radius="md">
                <Stack gap="xs">
                  <Text
                    className={styles.titleLink}
                    component={Link}
                    to={clientConfigure(p.id)}
                    fw={500}
                  >
                    {p.title}
                  </Text>
                  <Group gap="xs">
                    <Button component={Link} to={clientControl(p.id)} size="xs" variant="light">
                      Control
                    </Button>
                    <Button
                      variant="light"
        ...
        >
          <Stack>
            <TextInput
              ...
              autoFocus
            />
            <Button type="submit" loading={isCreating} disabled={!createTitle.trim() || isCreating}>
              Create
            </Button>
          </Stack>
        </form>
